...
export default function CheckoutPage({
  ...
  // Determine which ordering types are enabled
  const allOrderTypeOptions: OrderTypeOption[] = [
    {
      value: "dine_in" as const,
      label: "Dine In",
      description: "Eat at the restaurant",
      icon: <UtensilsCrossed className="size-5" />,
      enabled: mockSettings.ordering_dine_in,
    },
    {
      value: "takeaway" as const,
      label: "Takeaway / Collect",
      description: "Pick up your order",
      icon: <ShoppingBag className="size-5" />,
      enabled: mockSettings.ordering_takeaway,
    },
    {
      ...
      enabled: mockSettings.ordering_delivery,
    },
    {
      value: "drive_thru" as const,
      label: "Drive-Thru",
      description: "Stay in your vehicle",
      icon: <Car className="size-5" />,
      enabled: mockSettings.ordering_drive_thru,
    },
  ];
  const orderTypeOptions: OrderTypeOption[] = allOrderTypeOptions.filter((o) => o.enabled);

  const allPaymentOptions: PaymentOption[] = [
    {
      value: "cash" as const,
      label: "Cash",
      description: "Pay at counter / delivery",
      icon: <Banknote className="size-5" />,
      enabled: mockSettings.payment_cash,
    },
    {
      value: "card_venue" as const,
      label: "Card at Venue",
      description: "Card machine available",
      icon: <CreditCard className="size-5" />,
      enabled: mockSettings.payment_card_venue,
    },
    {
      ...
      enabled: mockSettings.payment_card_online,
    },
    {
      ...
      enabled: mockSettings.payment_bank_transfer,
    },
    {
      value: "jazzcash" as const,
      label: "JazzCash",
      description: "Redirect to JazzCash",
      icon: <Smartphone className="size-5" />,
      enabled: mockSettings.payment_jazzcash,
    },
    {
      value: "easypaisa" as const,
      label: "Easypaisa",
      description: "Redirect to Easypaisa",
      icon: <Smartphone className="size-5" />,
      enabled: mockSettings.payment_easypaisa,
    },
  ];
  const paymentOptions: PaymentOption[] = allPaymentOptions.filter((o) => o.enabled);

  // Determine starting step — skip step 1 if only one order type
  ...
  const initialOrderType: OrderType =
    enabledOrderTypes.length === 1 ? enabledOrderTypes[0].value : "dine_in";
  ...
  // Total steps (skip order type step if only one type)
  ...
  // Guest memory: detect returning customer on phone entry
  useEffect(() => {
    if (watchedPhone.replace(/\D/g, "").length >= 10) {
  ...
  // Customer info form data (stored between steps)
  ...
  const onCustomerInfoSubmit = (data: CustomerFormValues) => {
  ...
  const handlePlaceOrder = async () => {
    ...
    // Store order type in sessionStorage for order tracking page
  ...
  const renderStep1 = () => (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-semibold">How would you like to order?</h2>
        <p className="text-sm text-muted-foreground mt-1">
          Choose your preferred ordering type
        </p>
      </div>
    ...
    <form id="customer-info-form" onSubmit={handleSubmit(onCustomerInfoSubmit)}>
      <div className="space-y-4">
          ...
          {showReturningBanner && (
            ...
            >
              <p className="font-semibold text-primary">
                Welcome back, {mockReturningCustomer.name}! 🎉
              </p>
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => toast.info("Order your usual — coming soon!")}
              >
                Order your usual?
              </Button>
              {/* Saved address chips */}
              <div className="flex flex-wrap gap-2">
                {mockReturningCustomer.savedAddresses.map((addr) => (
                  <button
                    key={addr.label}
                    type="button"
                    onClick={() => setValue("delivery_address", addr.address)}
        ...
        <div className="space-y-4">
          ...
          {mockSettings.customer_require_name && (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                {...register("name", {
                  required: mockSettings.customer_require_name
                    ? "Name is required"
                    : false,
                })}
              />
          ...
          {mockSettings.customer_require_phone && (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                {...register("phone", {
                  required: mockSettings.customer_require_phone
                    ? "Phone is required"
                    : false,
                  minLength: {
                    value: 7,
          ...
          {mockSettings.customer_require_email && (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                {...register("email", {
                  required: mockSettings.customer_require_email
                    ? "Email is required"
                    : false,
                  pattern: {
                    value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
          ...
          {/* Car description — only for drive_thru */}
          {mockSettings.customer_require_car && orderType === "drive_thru" && (
            <div className="space-y-1.5">
              ...
              {errors.car_description && (
                <p className="text-xs text-destructive">
                  {errors.car_description.message}
                </p>
              )}
            </div>
          )}
              ...
              >
                ...
                {opt.value === "jazzcash" && (
                  <Button
                    ...
                    className="w-full"
                    onClick={() =>
                      toast.info("JazzCash redirect — coming soon!")
                    }
                  >
                ...
                {opt.value === "easypaisa" && (
                  <Button
                    ...
                    className="w-full"
                    onClick={() =>
                      toast.info("Easypaisa redirect — coming soon!")
                    }
                  >
  ...
  const renderStep4 = () => {
    ...
    const orderTypeLabel =
      orderTypeOptions.find((o) => o.value === orderType)?.label ?? orderType;
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          ...
          <div className="px-4 py-3 bg-muted/20 space-y-1.5 border-t border-border">
            ...
            <div className="flex justify-between text-sm text-muted-foreground">
              <span>Tax ({mockSettings.payment_tax_rate}%)</span>
          ...
          <div className="px-4 py-3 space-y-1.5 text-sm">
            ...
            {customerData.table_number && (
              <div className="flex justify-between">
                ...
                <span className="font-medium">#{customerData.table_number}</span>
              </div>
            )}
  ...
  const renderStepIndicator = () => {
    const labels = skipOrderTypeStep
      ? ["Your Details", "Payment", "Review"]
      : ["Order Type", "Your Details", "Payment", "Review"];
  ...
  const renderNav = () => {
    const isFirstStep =
      step === 1 || (skipOrderTypeStep && step === 2);
