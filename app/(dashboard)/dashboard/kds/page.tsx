    ...
    items: [
      {
        ...
        quantity: 2,
        unit_price: 1299,
        modifiers: [{ name: "Large", price_delta: 200 }],
        line_total: 2998,
      },
      {
        item_id: "i4",
    ...
    items: [
      {
        ...
        quantity: 1,
        unit_price: 1199,
        modifiers: [{ name: "Extra Spicy", price_delta: 0 }],
        line_total: 1199,
      },
      {
        item_id: "i6",
        ...
        quantity: 2,
        unit_price: 699,
        modifiers: [{ name: "Chocolate", price_delta: 0 }],
        line_total: 1398,
      },
    ],
    subtotal: 2597,
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        quantity: 2,
        unit_price: 799,
        modifiers: [{ name: "Ice Cream", price_delta: 150 }],
        line_total: 1898,
      },
    ],
    ...
    items: [
      {
        ...
        quantity: 1,
        unit_price: 1499,
        modifiers: [{ name: "No Pickles", price_delta: 0 }],
        line_total: 1499,
      },
      {
        item_id: "i4",
        ...
        quantity: 1,
        unit_price: 499,
        modifiers: [{ name: "Loaded (Cheese + Jalapeño)", price_delta: 300 }],
        line_total: 799,
      },
      {
        item_id: "i7",
    ...
    items: [
      {
        ...
        quantity: 2,
        unit_price: 1199,
        modifiers: [{ name: "Medium Spicy", price_delta: 0 }],
        line_total: 2398,
      },
    ],
    subtotal: 2398,
...
function isOverdue(mins: number): boolean {
  return mins >= 15;
}
...
interface EightySixPopoverProps {
  itemName: string;
}

function EightySixPopover({ itemName }: EightySixPopoverProps) {
  const [open, setOpen] = useState(false);

  function handleConfirm() {
    toast.success(`${itemName} marked unavailable`);
    setOpen(false);
  }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        className="ml-1 inline-flex items-center justify-center rounded px-1.5 py-0.5 text-[10px] font-bold leading-none bg-zinc-700 text-zinc-400 hover:bg-red-900/60 hover:text-red-300 transition-colors cursor-pointer border border-transparent hover:border-red-700/40"
        aria-label={`Mark ${itemName} as 86`}
      >
        86
      </PopoverTrigger>
      <PopoverContent className="w-56 bg-zinc-900 border-zinc-700 text-white p-3">
        ...
        <p className="text-xs text-zinc-400 mb-3">
          &ldquo;{itemName}&rdquo; will be 86&apos;d for this session.
        </p>
...
function OrderCard({ order, elapsedMins, onAction }: OrderCardProps) {
  ...
  // Ready column auto-fade: > 3 minutes in ready state
  const readyElapsed =
    order.status === "ready" && order.ready_at
      ? minutesSince(order.ready_at)
      : 0;
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        overdue
          ? "border-red-500/60 shadow-[0_0_12px_2px_rgba(239,68,68,0.25)]"
          : "border-zinc-800"
      )}
    >
      {/* Header */}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span
            className={cn(
              ...
              orderTypeBadgeClass(order.order_type)
            )}
          >
          ...
          {order.table_number && (
            <span className="rounded-full bg-zinc-700/60 px-2 py-0.5 text-xs font-semibold text-zinc-200">
              {order.table_number}
            </span>
          )}
        </div>
            ...
            {item.modifiers.length > 0 && (
              <div className="flex flex-col gap-0.5 pl-3">
                {item.modifiers.map((mod, mi) => (
                  <span
                    key={mi}
                    className="text-xs text-zinc-400 before:content-['+_'] before:text-zinc-600"
                  >
                    {mod.name}
                  </span>
                ))}
      ...
      <div className="flex items-center justify-between gap-2">
        <button
          ...
          className={cn(
            "min-h-[44px] flex-1 rounded-lg px-3 py-2 text-sm font-semibold transition-colors",
            actionClass
          )}
        >
          {actionLabel}
...
function KanbanColumn({
  ...
  return (
    <div className="flex flex-col min-h-0">
      ...
      <div
        className={cn(
          "flex items-center justify-between px-4 py-3 rounded-t-xl border-b border-zinc-800",
          headerClass
        )}
      >
        ...
        <span className="inline-flex items-center justify-center min-w-[24px] h-6 rounded-full bg-white/10 px-2 text-xs font-bold text-white">
          {col.length}
        </span>
      </div>
        ...
        <AnimatePresence initial={false} mode="popLayout">
          {col.map((order) => (
            <OrderCard
              key={order.id}
              order={order}
              elapsedMins={elapsedMap[order.id] ?? 0}
              onAction={onAction}
            />
          ))}
          {col.length === 0 && (
...
export default function KDSPage() {
  ...
  // Timer re-render — every 30s
  useEffect(() => {
    const id = setInterval(() => setTick((t) => t + 1), 30_000);
    return () => clearInterval(id);
  }, []);

  // Build elapsed map (recalculated on each tick)
  const elapsedMap = useCallback(() => {
    const map: Record<string, number> = {};
    for (const o of orders) {
      map[o.id] = getElapsedMins(o);
    }
    return map;
  ...
  function handleAction(orderId: string) {
    setOrders((prev) =>
      prev.map((o) => {
        ...
        if (o.status === "pending") {
          return { ...o, status: "preparing" as OrderStatus, accepted_at: now, preparing_at: now };
        }
        if (o.status === "preparing") {
          return { ...o, status: "ready" as OrderStatus, ready_at: now };
        }
        if (o.status === "ready") {
  ...
  function handleAudioToggle(checked: boolean) {
    ...
    if (checked) {
      toast.info("Audio alerts require browser permission");
    }
  }
  ...
  return (
    <div
      className={cn(
        ...
        extraDark ? "bg-black" : "bg-zinc-950"
      )}
    >
      {/* ── Top Bar ── */}
      <header className="flex-none flex items-center justify-between gap-4 px-4 py-2.5 border-b border-zinc-800 bg-zinc-950/90 backdrop-blur-sm z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <Select value={branch} onValueChange={(v) => { if (v !== null) setBranch(v); }}>
            <SelectTrigger
              className="h-8 min-w-[130px] border-zinc-700 bg-zinc-800/60 text-zinc-200 text-xs"
              size="default"
            >
              <SelectValue placeholder="Branch" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="b1">Main Branch</SelectItem>
              <SelectItem value="b2">Downtown</SelectItem>
              <SelectItem value="b3">Airport</SelectItem>
            </SelectContent>
          </Select>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Switch
              checked={extraDark}
              onCheckedChange={setExtraDark}
              size="sm"
              aria-label="Extra dark mode"
            />
          </div>
          ...
          >
            ...
            <span className="text-xs">Exit KDS</span>
          </Button>
        </div>
      </header>
